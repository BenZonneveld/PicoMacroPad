...
static void LCD_InitReg(void)
{
	...
	if(LCD_2_8 == id){
		...
		LCD_WriteReg(0xb7); // GCTRL
		LCD_WriteData(0x35);
		
		LCD_WriteReg(0xbb); // VCOMS
		LCD_WriteData(0x28);
		
		LCD_WriteReg(0xc0); // LCMCTRL
		LCD_WriteData(0x3c);
		
		LCD_WriteReg(0xc2); // VDVVRHEN
		LCD_WriteData(0x01);
		
		LCD_WriteReg(0xc3); // VRHS
		LCD_WriteData(0x0b);
		
		LCD_WriteReg(0xc4); // VDVS
		LCD_WriteData(0x20);
		
		LCD_WriteReg(0xc6); // FRCTRL2
		LCD_WriteData(0x0f);
		
		LCD_WriteReg(0xD0); // PWCTRL1
		LCD_WriteData(0xa4);
		LCD_WriteData(0xa1);
		
		LCD_WriteReg(0xe0); // PVGAMCTRL
		LCD_WriteData(0xd0);
		LCD_WriteData(0x01);
		LCD_WriteData(0x08);
		LCD_WriteData(0x0f);
