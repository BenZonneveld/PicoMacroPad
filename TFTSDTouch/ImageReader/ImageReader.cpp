...
#else
#define BUFPIXELS 320 ///< 200 * 5 = 1000 bytes
#endif
...
//                    filesys->f_read(&file, NULL, 1, NULL);
                    b = g = r = 0;
//                    b = getc(&file);// (&file.read();
//                    g = getc(&file);// .read();
//                    r = getc(&file);//file.read();
//                    (void)getc(&file);//file.read(); // Ignore 4th byte
                  quantized[c] =
              ...
              for (row = 0; row < loadHeight; row++) { // For each scanline...
                ...
                for (col = 0; col < loadWidth; col++) { // For each pixel...
                  if (srcidx >= sizeof sdbuf) {         // Time to load more?
                    if (tft) {                          // Drawing to TFT?
                      filesys->f_read(&file, &sdbuf, sizeof sdbuf , NULL); // Load from SD
                      if (destidx) {       // If buffered TFT data
                        ...
                        filesys->f_read(&file, &sdbuf, sizeof sdbuf, NULL);
                ...
                if (tft) {       // Drawing to TFT?
                  if (destidx) { // Any remainders?
                    ...
                    tft->writePixels(dest, destidx); // Write it
                    destidx = 0; // and reset dest index
                  }
                }
              } // end scanline loop
