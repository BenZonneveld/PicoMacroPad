...
#include "MMC_SD.h"		/* Declarations of disk I/O functions */
...
void MMC_SD::mem_cpy (void* dst, const void* src, UINT cnt) {
	BYTE *d = (BYTE*)dst;
...
/* Fill memory */
void MMC_SD::mem_set (void* dst, int val, UINT cnt) {
	BYTE *d = (BYTE*)dst;
...
int MMC_SD::mem_cmp (const void* dst, const void* src, UINT cnt) {
...
int MMC_SD::chk_chr (const char* str, int chr) {
...
FRESULT MMC_SD::chk_lock (	/* Check if the file can be accessed */
...
int MMC_SD::enq_lock (void)	/* Check if an entry is available for a new object */
{
	UINT i;
...
UINT MMC_SD::inc_lock (	/* Increment object open counter and returns its index (0:Internal error) */
...
FRESULT MMC_SD::dec_lock (	/* Decrement object open counter */
...
void MMC_SD::clear_lock (	/* Clear lock entries of the volume */
	FATFS *fs
)
{
	UINT i;
...
FRESULT MMC_SD::sync_fs (	/* FR_OK: successful, FR_DISK_ERR: failed */
	FATFS* fs		/* File system object */
)
{
	FRESULT res;
...
DWORD MMC_SD::clust2sect (	/* !=0: Sector number, 0: Failed - invalid cluster# */
	FATFS* fs,		/* File system object */
...
DWORD MMC_SD::get_fat (	/* 0xFFFFFFFF:Disk error, 1:Internal error, Else:Cluster status */
	FATFS* fs,	/* File system object */
...
DWORD MMC_SD::create_chain (	/* 0:No free cluster, 1:Internal error, 0xFFFFFFFF:Disk error, >=2:New cluster# */
	FATFS* fs,			/* File system object */
...
#if _USE_FASTSEEK
DWORD MMC_SD::clmt_clust (	/* <2:Error, >=2:Cluster number */
...
FRESULT MMC_SD::dir_next (	/* FR_OK:Succeeded, FR_NO_FILE:End of table, FR_DENIED:Could not stretch */
...
int MMC_SD::cmp_lfn (			/* 1:Matched, 0:Not matched */
...
int MMC_SD::pick_lfn (			/* 1:Succeeded, 0:Buffer overflow */
...
/*-----------------------------------------------------------------------*/
#if !_FS_READONLY
FRESULT MMC_SD::dir_register (	/* FR_OK:Successful, FR_DENIED:No free entry or too many SFN collision, FR_DISK_ERR:Disk error */
...
FRESULT MMC_SD::dir_remove (	/* FR_OK: Successful, FR_DISK_ERR: A disk error */
...
void MMC_SD::get_fileinfo (		/* No return code */
	DIR* dp,			/* Pointer to the directory object */
...
int MMC_SD::get_ldnumber (		/* Returns logical drive number (-1:invalid drive) */
...
FRESULT MMC_SD::follow_path (	/* FR_OK(0): successful, !=0: error code */
...
BYTE MMC_SD::check_fs (	/* 0:FAT boor sector, 1:Valid boor sector but not FAT, 2:Not a boot sector, 3:Disk error */
	FATFS* fs,	/* File system object */
...
FRESULT MMC_SD::find_volume (	/* FR_OK(0): successful, !=0: any error occurred */
...
FRESULT MMC_SD::validate (	/* FR_OK(0): The object is valid, !=0: Invalid */
