...
unsigned char MMC_SD::SD_SPI_ReadWriteByte(unsigned char CMD)
{
...
void MMC_SD::SD_SPI_SpeedLow(void)
{
//	SPI1->CR1&=0XFFC7; 
...
void MMC_SD::SD_SPI_SpeedHigh(void)
{
//	SPI1->CR1&=0XFFC7; 
...
unsigned char MMC_SD::SD_Select(void)
{
...
unsigned char MMC_SD::SD_WaitReady(void)
{
	unsigned int t=0;
	do{
...
unsigned char MMC_SD::SD_GetResponse(unsigned char Response)
{
...
//return: 0 for success, other for failure.   
unsigned char MMC_SD::SD_RecvData(unsigned char *buf,unsigned short len)
{			  	  
...
//cmd: command
//return: 0 for success, other for failure.   
unsigned char MMC_SD::SD_SendBlock(unsigned char*buf,unsigned char cmd)
{	
	unsigned short t;		  	  
...
unsigned char MMC_SD::SD_SendCmd(unsigned char cmd, unsigned int arg, unsigned char crc)
{
    unsigned char r1;	
...
unsigned char MMC_SD::SD_GetCID(unsigned char *cid_data)
{
    unsigned char r1;	   
...
//return: 0 no error  1 error
unsigned char MMC_SD::SD_GetCSD(unsigned char *csd_data)
{
    unsigned char r1;	 
...
uint32_t MMC_SD::SD_GetSectorCount(void)
{
...
//initialize sd card 
unsigned char MMC_SD::SD_Initialize(void)
{
    unsigned char r1;      
...
uint8_t MMC_SD::SD_ReadDisk(uint8_t *buf,uint32_t sector,uint8_t cnt)
{
	unsigned char r1;
...
//cnt: totals of sectors]
//return: 0 ok, other for failure
uint8_t MMC_SD::SD_WriteDisk(uint8_t *buf,uint32_t sector,uint8_t cnt)
{
	unsigned char r1;
