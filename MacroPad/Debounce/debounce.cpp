...
int Debounce::debounce_gpio(uint gpio)
{
    ...
    else
    {
        ...
        offset[gpio] = pio_add_program(pio_debounced[gpio], &debounce_program);
        // make a sm config
        conf[gpio] = debounce_program_get_default_config(offset[gpio]);
...
int Debounce::read(uint gpio)
{
    ...
    if (pc >= (offset[gpio] + debounce_border))
        return 1;
    else
        return 0;
};
...
int Debounce::undebounce_gpio(uint gpio)
{
    ...
    {
        // remove the program
        pio_remove_program(pio_used, &debounce_program, offset_used);
