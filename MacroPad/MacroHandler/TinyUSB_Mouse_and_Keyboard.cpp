...
//    while(!tud_hid_ready()) sleep_ms(1);
    tud_hid_mouse_report(RID_MOUSE,_buttons,x,y,wheel,0);
    tud_task();
  }
...
//    while( !tud_mounted() ) sleep_ms(1);
  ...
  void TinyKeyboard_::sendReport(KeyReport* keys)
  {
    ...
    while(!tud_hid_ready()) sleep_ms(1);
    tud_hid_keyboard_report(RID_KEYBOARD,keys->modifiers,keys->keys);
    sleep_ms(10);
    tud_task();
  }
